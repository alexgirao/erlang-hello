...
main(_Args) ->
    true = code:add_patha("ebin"),  % required for ebin/erlybank.appup
    true = code:add_patha("../part6/ebin"), % required for erlybank.app version "1.0"
	...
	"1" ->
	    % this comes from ../part6
	    ok = upgrade(PermanentRelease);
	Found ->
	    ?log_err1("release ~p not available/permanent, found ~p instead", ["1", Found])
    end,
