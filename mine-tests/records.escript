...
% all records are tagged by its name, but not the tag structure

-record(person, {a, b}).

% is_record is a build-in-function (BIF) and available only at compile time
...
main(_) ->
    ...
    ,io:format("record field: ~p~n", [P0#person.a])
